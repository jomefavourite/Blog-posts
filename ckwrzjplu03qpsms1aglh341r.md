...
Hello there and welcome back to the [Let's Build something cool series](https://favouritejome.hashnode.dev/series/lets-build-something-cool). 
 In this article, we'll build the Windows 11 dropdown that's displayed on the desktop. Here's a preview:
...
We'll need the icons Radix provides also, so let's install the icon library.
...
![screely-1638434371517.png](https://cdn.hashnode.com/res/hashnode/image/upload/v1638434395933/2hg3Ewxwm.png)

### How to get the Dropdown setup

Before getting started with the windows dropdown component, let's go through a demo of how the Radix dropdown will be set up.
...
From the demo above you'll notice the dropdown doesn't have styles applied. Try navigating through the items with the keyboard arrows *(up key and down key)* which the button is clicked, you'll notice the first item is focused, then when moved to the next item it's also focused well this will only work on a desktop not mobile. This shows how accessible the library is.
...
Styling the component is very easy, all we need is to apply classes `className` to each component we'll like to style. Let's see this in action:
